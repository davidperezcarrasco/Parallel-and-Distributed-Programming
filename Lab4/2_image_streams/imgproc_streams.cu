...
int main (int argc, char *argv[])
{
  ...
  /* Print runtime */
   ...
   cudaMemcpy(image, d_image, sizeof(int) * nx * ny, cudaMemcpyHostToDevice);
   
   invert<<<dimGrid, dimBlock, sizeof(int) * B_size * B_size, stream1>>>(d_image, d_image_invert, nx, ny);
   smooth<<<dimGrid, dimBlock, sizeof(int) * B_size * B_size, stream2>>>(d_image, d_image_smooth, nx, ny);
   detect<<<dimGrid, dimBlock, sizeof(int) * B_size * B_size, stream3>>>(d_image, d_image_detect, nx, ny);
   enhance<<<dimGrid, dimBlock, sizeof(int) * B_size * B_size, stream4>>>(d_image, d_image_enhance, nx, ny);
