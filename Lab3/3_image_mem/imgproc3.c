...
/*invert*/
void invert(int* image, int* image_invert, int nx, int ny, int start, int end, int block_size){
   #pragma acc parallel loop copyin(image[0:nx*ny]) copyout(image_invert[0:nx*ny])
   for (int j = start / block_size; j < end / block_size; j++){
      for (int i = 0; i < nx; i++){
...
void smooth(int* image, int* image_smooth, int nx, int ny){
   #pragma acc parallel loop copyout(image_smooth[0:nx*ny])
   for(int j=0; j<ny; ++j){
   ...
   #pragma acc parallel loop copyin(image[0:nx*ny], kernel[0:9]) copy(image_smooth[0:nx*ny])
...
void detect(int* image, int* image_detect, int nx, int ny){
   #pragma acc parallel loop copyout(image_detect[0:nx*ny])
   for(int j=0; j<ny; ++j){
   ...
   #pragma acc parallel loop copyin(image[0:nx*ny], kernel[0:9]) copy(image_detect[0:nx*ny])
   for (int j = 1; j < ny - 1; j++){
      for (int i = 1; i < nx - 1; i++){
...
void enhance(int* image,int *image_enhance,int nx, int ny){
   #pragma acc parallel loop copyout(image_enhance[0:nx*ny])
   for(int j=0; j<ny; ++j){
   ...
   #pragma acc parallel loop copyin(image[0:nx*ny], kernel[0:9]) copy(image_enhance[0:nx*ny])
   for (int j = 1; j < ny - 1; j++){
      for (int i = 1; i < nx - 1; i++){
...
int main (int argc, char *argv[])
{
   ...
   double runtime;
   /* Apply filters */
   #pragma acc data copyin(image[0:nx*ny]) copyout(image_invert[0:nx*ny], image_smooth[0:nx*ny], image_detect[0:nx*ny], image_enhance[0:nx*ny])
   {
      double init_time = omp_get_wtime();
      int start, end;
      int block_size = nx * ny / 4; 
      for (int iter = 0; i < 4; i++){
         start = block_size * iter;
         end = block_size * (iter + 1);
         invert(image, image_invert, nx, ny, start, end, block_size);
         smooth(image, image_smooth, nx, ny, start, end);
         detect(image, image_detect, nx, ny, start, end);
         enhance(image, image_enhance, nx, ny, start, end);
      }
