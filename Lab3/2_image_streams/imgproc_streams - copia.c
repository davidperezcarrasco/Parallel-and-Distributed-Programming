...
void invert(int* image, int* image_invert, int nx, int ny){
   #pragma acc parallel loop copyin(image[0:nx*ny]) copyout(image_invert[0:nx*ny]) async
   for (int j = 0; j < ny; j++){
      for (int i = 0; i < nx; i++){
         image_invert[pixel(i, j, nx)] = 255 - image[pixel(i, j, nx)];
      }
   }
   #pragma acc update self(image_invert[0:nx*ny])
}
...
void smooth(int* image, int* image_smooth, int nx, int ny){
   ...
   #pragma acc parallel loop copyin(image[0:nx*ny]) copy(image_smooth[0:nx*ny])
   ...
   #pragma acc update self(image_smooth[0:nx*ny])
}
...
void detect(int* image, int* image_detect, int nx, int ny){
   ...
   #pragma acc parallel loop copyin(image[0:nx*ny]) copy(image_detect[0:nx*ny])
   for (int j = 1; j < ny - 1; j++){
      for (int i = 1; i < nx - 1; i++){
   ...
   #pragma acc update self(image_detect[0:nx*ny])
}
...
void enhance(int* image,int *image_enhance,int nx, int ny){
  #pragma acc parallel loop copyout(image_enhance[0:nx*ny])
   ...
   #pragma acc parallel loop copyin(image[0:nx*ny]) copy(image_enhance[0:nx*ny])
   for (int j = 1; j < ny - 1; j++){
      for (int i = 1; i < nx - 1; i++){
   ...
   #pragma acc update self(image_enhance[0:nx*ny])
}
...
int main (int argc, char *argv[])
{
   ...
   #pragma acc update self(image[0:nx*ny])
